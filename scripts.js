...
function renderTree() {
  ...
  container.innerHTML = ''; // Clear the container before rendering new tree
  const root = getRoot(); // Get the root person (first generation)
...
function positionTree(person, x, y) {
  ...
  if (person.relations.children.length > 0) {
    ...
    person.relations.children.forEach(childId => {
      ...
      childX += nodeWidth + horizontalSpacing; // Adjust horizontal spacing
    });
  }
}
